    ...
    <div v-if="isLoading" class="max-w-4xl mx-auto py-6 px-4 sm:px-6 lg:px-8">
      <div class="bg-white dark:bg-[#0a0a0a] shadow rounded-lg p-6">
        <div class="animate-pulse">
          <div class="h-8 bg-gray-200 dark:bg-gray-800 rounded w-1/2 mb-4"></div>
          <div class="space-y-3">
    ...
    <div v-else class="max-w-4xl mx-auto py-6 px-4 sm:px-6 lg:px-8">
      ...
      <div class="bg-white dark:bg-[#0a0a0a] shadow rounded-lg p-6 mb-6">
        <div class="flex items-center justify-between mb-4">
          <button @click="goBack" class="text-gray-500 dark:text-gray-400 hover:text-gray-700 dark:hover:text-gray-300">
        ...
        <div class="flex items-center justify-between">
          <div>
            ...
            <p class="mt-2 text-gray-600 dark:text-gray-300">{{ lesson.description }}</p>
          </div>
      ...
      <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div class="lg:col-span-2 space-y-6">
          <!-- Theory Section -->
          <div class="bg-white dark:bg-[#0a0a0a] shadow rounded-lg p-6">
            <h2 class="text-xl font-semibold text-gray-900 dark:text-white mb-4">Lý thuyết</h2>
          ...
          <!-- Examples Section -->
          <div class="bg-white dark:bg-[#0a0a0a] shadow rounded-lg p-6">
            <h2 class="text-xl font-semibold text-gray-900 dark:text-white mb-4">Ví dụ</h2>
            <div class="space-y-4">
              <div v-for="example in lesson.examples" :key="example.id" class="border-l-4 border-blue-500 pl-4">
                ...
                <p class="text-gray-600 dark:text-gray-300 text-sm mt-1">{{ example.translation }}</p>
                <p class="text-gray-500 dark:text-gray-400 text-xs mt-1" v-if="example.note">{{ example.note }}</p>
              </div>
            </div>
          </div>
          ...
          <!-- Usage Rules -->
          <div class="bg-white dark:bg-[#0a0a0a] shadow rounded-lg p-6">
            <h2 class="text-xl font-semibold text-gray-900 dark:text-white mb-4">Quy tắc sử dụng</h2>
            <div class="space-y-3">
              <div v-for="rule in lesson.rules" :key="rule.id" class="flex items-start space-x-3">
                <div class="flex-shrink-0 w-6 h-6 bg-blue-100 dark:bg-gray-800 rounded-full flex items-center justify-center">
                ...
                <div>
                  ...
                  <p class="text-gray-600 dark:text-gray-300 text-sm">{{ rule.description }}</p>
                </div>
              </div>
            </div>
          </div>
          ...
          <div v-if="lesson.commonMistakes && lesson.commonMistakes.length > 0" class="bg-white dark:bg-[#0a0a0a] shadow rounded-lg p-6">
            <h2 class="text-xl font-semibold text-gray-900 dark:text-white mb-4">Lỗi thường gặp</h2>
            <div class="space-y-4">
              <div v-for="mistake in lesson.commonMistakes" :key="mistake.id" class="bg-red-50 dark:bg-gray-800 border border-red-200 dark:border-red-900 rounded-lg p-4">
                <div class="flex items-start space-x-3">
                  ...
                  <div>
                    ...
                    <p class="text-gray-600 dark:text-gray-300 text-sm mt-1">{{ mistake.explanation }}</p>
                  </div>
                </div>
              </div>
            </div>
        ...
        <div class="space-y-6">
          <!-- Quick Info -->
          <div class="bg-white dark:bg-[#0a0a0a] shadow rounded-lg p-6">
            <h3 class="text-lg font-medium text-gray-900 dark:text-white mb-4">Thông tin</h3>
            <div class="space-y-3">
              <div class="flex justify-between">
                <span class="text-gray-600 dark:text-gray-400">Chủ đề:</span>
                <span class="text-gray-900 dark:text-white">{{ lesson.category }}</span>
              </div>
              <div class="flex justify-between">
                <span class="text-gray-600 dark:text-gray-400">Thời gian:</span>
                <span class="text-gray-900 dark:text-white">{{ lesson.duration }} phút</span>
              </div>
              <div class="flex justify-between">
                <span class="text-gray-600 dark:text-gray-400">Bài tập:</span>
                <span class="text-gray-900 dark:text-white">{{ lesson.exercises }} bài</span>
              </div>
              <div class="flex justify-between">
                <span class="text-gray-600 dark:text-gray-400">Đánh giá:</span>
          ...
          <!-- Related Lessons -->
          <div class="bg-white dark:bg-[#0a0a0a] shadow rounded-lg p-6">
            <h3 class="text-lg font-medium text-gray-900 dark:text-white mb-4">Bài học liên quan</h3>
            <div class="space-y-3">
              <div v-for="related in relatedLessons" :key="related.id" class="border border-gray-200 dark:border-gray-800 rounded-lg p-3 hover:bg-gray-50 dark:hover:bg-gray-800 cursor-pointer" @click="viewRelatedLesson(related)">
          ...
          <!-- Quick Actions -->
          <div class="bg-white dark:bg-[#0a0a0a] shadow rounded-lg p-6">
            <h3 class="text-lg font-medium text-gray-900 dark:text-white mb-4">Hành động</h3>
            <div class="space-y-3">
