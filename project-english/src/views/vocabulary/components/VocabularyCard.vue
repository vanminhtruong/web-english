<template>
  <div>
    <div class="p-6 hover:bg-gray-50 dark:hover:bg-gray-700 cursor-pointer" @click="viewDetails">
      <div class="flex items-center justify-between">
        <div class="flex-1">
          <div class="flex items-center space-x-4">
            <!-- Image Display -->
            <div v-if="word.image" class="flex-shrink-0">
              <div class="w-20 h-20 rounded-lg overflow-hidden border-2 border-gray-200 dark:border-gray-600">
                <img
                  :src="word.image"
            ...
            <div class="flex-1">
              <h4 class="text-lg font-semibold text-gray-900 dark:text-white">{{ word.word }}</h4>
              <p class="text-sm text-gray-500 dark:text-gray-400">{{ word.pronunciation }}</p>
              <p class="text-sm text-gray-600 dark:text-gray-300 mt-1">{{ word.partOfSpeech }}</p>
              ...
              <div class="flex items-center space-x-4 mt-2 text-xs text-gray-500 dark:text-gray-400">
                <div class="flex items-center space-x-1" v-if="word.createdAt">
                  ...
                  <span>{{ t('vocabulary.createdAt') }}: {{ getRelativeTime(word.createdAt, $i18n.locale) }}</span>
                </div>
                <div class="flex items-center space-x-1" v-if="word.updatedAt && word.updatedAt !== word.createdAt">
                  ...
                  <span>{{ t('vocabulary.updatedAt') }}: {{ getRelativeTime(word.updatedAt, $i18n.locale) }}</span>
                </div>
              </div>
            </div>
