<template>
  <div>
    <div class="p-4 md:p-6 hover:bg-gray-50 dark:hover:bg-gray-700 cursor-pointer" @click="viewDetails">
      <div class="flex flex-col lg:flex-row lg:items-center lg:justify-between space-y-4 lg:space-y-0">
        <div class="flex-1">
          <div class="flex flex-col md:flex-row md:items-start space-y-4 md:space-y-0 md:space-x-4">
            <!-- Image Display -->
            <div v-if="word.image" class="flex-shrink-0 self-center md:self-start">
              <div class="w-16 h-16 md:w-20 md:h-20 rounded-lg overflow-hidden border-2 border-gray-200 dark:border-gray-600">
                <img
                  :src="word.image"
                  :alt="`${t('vocabulary.image.preview')} - ${word.word}`"
            ...
            <div class="flex-1 min-w-0">
              <h4 class="text-lg md:text-xl font-semibold text-gray-900 dark:text-white truncate">{{ word.word }}</h4>
              <p class="text-sm md:text-base text-gray-500 dark:text-gray-400">{{ word.pronunciation }}</p>
              <p class="text-sm md:text-base text-gray-600 dark:text-gray-300 mt-1">{{ word.partOfSpeech }}</p>
              <!-- Date time information -->
              <div class="flex flex-col sm:flex-row sm:items-center sm:space-x-4 mt-2 text-xs md:text-sm text-gray-500 dark:text-gray-400 space-y-1 sm:space-y-0">
                <div class="flex items-center space-x-1" v-if="word.createdAt">
                  <svg class="w-3 h-3" fill="currentColor" viewBox="0 0 20 20">
                    <path fill-rule="evenodd" d="M10 18a8 8 0 100-16 8 8 0 000 16zm1-12a1 1 0 10-2 0v4a1 1 0 00.293.707l2.828 2.829a1 1 0 101.415-1.415L11 9.586V6z" clip-rule="evenodd"/>
                  </svg>
                  <span class="truncate">{{ t('vocabulary.createdAt') }}: {{ getRelativeTime(word.createdAt, $i18n.locale) }}</span>
                </div>
                <div class="flex items-center space-x-1" v-if="word.updatedAt && word.updatedAt !== word.createdAt">
                  ...
                  <span class="truncate">{{ t('vocabulary.updatedAt') }}: {{ getRelativeTime(word.updatedAt, $i18n.locale) }}</span>
                </div>
              </div>
            </div>

            <div class="flex-1 md:flex-2 min-w-0">
              <p class="text-sm md:text-base text-gray-700 dark:text-gray-300">{{ word.meaning }}</p>
              <p class="text-sm md:text-base text-gray-500 dark:text-gray-400 mt-1" v-if="word.example">
                <span class="font-medium">{{ t('vocabulary.example') }}:</span> {{ word.example }}
              </p>
            </div>
            
            <div class="flex-shrink-0 flex flex-row md:flex-col items-center md:items-end space-x-2 md:space-x-0 md:space-y-2">
              <span :class="getLevelColor(word.level)" class="px-2 py-1 text-xs font-medium rounded-full whitespace-nowrap">
                {{ t(`vocabulary.levels.${word.level}`) }}
              </span>
              <p class="text-xs md:text-sm text-gray-500 truncate max-w-24 md:max-w-none">{{ getTopicName(word.category) }}</p>
            </div>
          </div>
        </div>
        
        <div class="flex items-center justify-center lg:justify-start space-x-2 lg:ml-4">
    ...
    <div v-if="isImageModalOpen" class="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-75" @click="closeImageModal">
        <div class="max-w-4xl max-h-4xl">
            <img :src="word.image" :alt="`${t('vocabulary.image.preview')} - ${word.word}`" class="max-w-full max-h-full rounded-lg">
        </div>
    </div>
  </div>
</template>
