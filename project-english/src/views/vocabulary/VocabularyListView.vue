          ...
          <div class="space-y-4 sm:space-y-6">
            ...
            <div class="bg-white/60 dark:bg-[#0a0a0a]/60 backdrop-blur-sm rounded-xl border border-white/20 dark:border-white/10 p-3 sm:p-4 md:p-5">
              ...
              <SaveControlPanel
                ...
                @import-google-drive-file="handleGoogleDriveImportWithReload"
              />
            </div>
          </div>
        </BaseAccordion>
...
const handleGoogleDriveFileImport = async (fileInfo: { fileId: string; fileName: string; size: string; modifiedTime: string }) => {
  const { t } = useI18n();
  const toast = useToast();
  
  try {
    // Import Google Drive file service
    const { GoogleDriveFileService } = await import('../../services/googleDriveFileService');
    ...
    const toastId = toast(
      {
        component: ConfirmToast,
        props: {
          message: t('vocabulary.save.import.confirmGoogleDriveMessage', { filename: fileInfo.fileName }, `Import ${fileInfo.fileName} from Google Drive? This will replace all current vocabulary.`),
          ...
          onConfirm: async () => {
            try {
              ...
              // Import vocabularies using the same logic as file import
              ...
              let importMessage = t('vocabulary.save.import.googleDriveSuccessMessage', { count: data.vocabularies.length }, `Successfully imported ${data.vocabularies.length} vocabulary words from Google Drive`);
              ...
              toast.error(t('vocabulary.save.errors.googleDriveImportFailed', 'Google Drive import failed') || `Google Drive import failed: ${(error as Error).message}`);
              isSaving.value = false;
            }
...
const handleGoogleDriveImportWithReload = (fileInfo: { fileId: string; fileName: string; size: string; modifiedTime: string }) => {
  const handleImportComplete = (event: CustomEvent) => {
    nextTick(() => {
      reloadGroupingState();
